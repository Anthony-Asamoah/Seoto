...
- Admin login reCAPTCHA: `infrastructure/core/admin_forms.py` holds `RecaptchaAdminLoginMixin` (verifies the token *before* `AuthenticationForm.clean()` authenticates, so the form can't be used to probe credentials) plus the non-OTP `AdminLoginForm`; the OTP pair lives in `infrastructure/core/admin.py` (`OTPAdminLoginForm`, wired as `SeotoAdminSite.login_form`) to keep django_otp out of import time. `SeotoAdminConfig.ready()` stamps its `login_form` attribute onto `admin.site`, so both the OTP and recovery paths are covered. The check itself is `infrastructure.core.mixins.views.enforce_recaptcha`, shared with `RecaptchaMixin` — patch `is_human` there in tests.
