...
- `infrastructure/utils/` — `media.py` (`MediaHelper` for thumbnail generation, used across foodie/accounts), `choices.py` (`BaseChoices`, the `TextChoices` base every model enum subclasses), `admin.py` (`RichTextAdminMixin`), `email.py`, `validators.py`, `profanity.py`. The package `__init__` re-exports everything but `contains_profanity`.
...
- htmx: `static/js/htmx.min.js` is vendored (2.0.10, from the npm tarball — record the version in its header comment when upgrading) and loaded site-wide from `base.html`. It is inert on pages with no `hx-*` attributes; there is no `hx-boost`. Vendored rather than CDN because `sw.js` already serves `/static/` cache-first, so it is offline-safe without a `STATIC_ASSETS` entry — but bump `CACHE_VERSION` when you change a JS file, since cache-first would otherwise pin the old copy on the `AWS` path (no manifest, so no hashed URL). `csrf.js` owns the `htmx:configRequest` hook that sets `X-CSRFToken`; don't add a second one.
  - The convention is **section swaps**: a view branches on `request.headers.get('HX-Request')` and re-renders one `id`-wrapped partial (`hx-target="#the-id" hx-swap="outerHTML"`), leaving the non-htmx redirect intact so the no-JS path still works. `spending_tracker.views._htmx_section` is the helper — it appends `partials/_toasts_oob.html`, because an htmx swap never reaches the full-page render where `messages` would otherwise appear. Guard clauses that redirect need `_htmx_redirect` (204 + `HX-Redirect`): htmx follows a 302 itself and would swap a whole page into the section.
  - `_messages.html` renders `#toast-tray` even when empty — an OOB swap silently no-ops if its target id is absent — and keeps the div's content whitespace-free so `#toast-tray:empty` can hide it. Its toast init re-runs on `htmx:load`/`htmx:oobAfterSwap`, guarded by `data-toast-bound`.
  - Per-element JS must be re-bindable: swapped-in nodes have no listeners. `recurring_list.html`'s swipe script is the pattern — an `init*` function that skips already-flagged elements, called at parse time and again on `htmx:load`.
  - `onsubmit="return confirm(...)"` does **not** gate an htmx request (htmx ignores the handler's return value). Use `hx-confirm`.
