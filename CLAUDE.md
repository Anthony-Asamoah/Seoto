...
- Admin skin: `jazzmin` (Bootstrap 5.3 + AdminLTE 4) — it must stay ahead of the admin app in `INSTALLED_APPS`. Branding lives in `JAZZMIN_SETTINGS`; the silver/brown palette is applied through Bootstrap CSS custom properties in `static/css/admin.css` (`custom_css`), since jazzmin 3.x dropped the AdminLTE `accent-*` / `sidebar-dark-*` classes that `JAZZMIN_UI_TWEAKS` used to drive. `templates/admin/login.html` is a copy of jazzmin's with the `otp_token` field and the `partials/_recaptcha.html` include added — re-sync it if jazzmin's login template changes.
- Admin login reCAPTCHA: `common/admin_forms.py` holds `RecaptchaAdminLoginMixin` (verifies the token *before* `AuthenticationForm.clean()` authenticates, so the form can't be used to probe credentials) plus the non-OTP `AdminLoginForm`; the OTP pair lives in `common/admin.py` (`OTPAdminLoginForm`, wired as `SeotoAdminSite.login_form`) to keep django_otp out of import time. `SeotoAdminConfig.ready()` stamps its `login_form` attribute onto `admin.site`, so both the OTP and recovery paths are covered. The check itself is `common.mixins.views.enforce_recaptcha`, shared with `RecaptchaMixin` — patch `is_human` there in tests.
